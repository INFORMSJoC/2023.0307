...
class Node(object):
    ...
    def expanded(self) -> bool:  # Whether it has been extended
        return len(self.children) > 0

    def value(self) -> float:
        if self.visit_count == 0:
            return 0
        return self.value_sum / self.visit_count  # return mean of value
    ...
    def expand(self, to_play, actions, network_output):
        ...
        for action, p in policy.items():
            self.children[action] = Node(p / policy_sum)  # prior prob of each action
...
class MCTS(object):
    ...
    def run(self, root, action_history, model):
        ...
        for _ in range(self.config.search_budget):
    ...
    def select_child(self, node, min_max_stats):
        if self.config.node_selection_policy == 'UCT':
            _, action, child = max((self.ucb_score(node, child, min_max_stats), action, child)
                                   for action, child in node.children.items())
        else:
            _, action, child = max((self.aoat_score(node, child), action, child)
                                   for action, child in node.children.items())
    ...
    def aoat_score(self, parent, child) -> float:
        if child.visit_count > 0:
            ms = np.mean(child.value())
            ns = child.visit_count
            vas = np.var(child.value())
            pv = 1 / (1 / self.config.sigmaa_0 + ns / vas)
            pm = pv * (parent.value() / self.config.sigmaa_0 + ns * ms / vas)
        else:
            pm = parent.value()
        return pm
