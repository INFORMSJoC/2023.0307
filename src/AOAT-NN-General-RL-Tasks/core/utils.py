...
def select_action(node, policy, temperature=1, deterministic=True):
    ...
    if deterministic:
        if policy == 'UCT':
            ...
            action_pos = np.argmax([child.value() for action, child in node.children.items()])
        else:
            weighted_values = [child.value() * child.prior for action, child in node.children.items()]
            action_pos = np.argmax(weighted_values)
    else:
